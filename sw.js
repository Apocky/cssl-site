 ...
 *
 * Strategy :
 *   precache      : index.html, /CSLv3, /js/*, /manifest.webmanifest
  ...
  e.waitUntil(
    caches.open(CACHE).then(function (c) {
      return c.addAll(PRECACHE).catch(function () {
        // individual failures are fine; progressive enhancement
...
self.addEventListener('fetch', function (e) {
  ...
  if (url.origin !== location.origin) return;   // only cache same-origin
